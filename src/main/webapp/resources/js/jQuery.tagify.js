    ...
    Tagify.prototype = {
        ...
        callbacks : {
            ...
            onKeydown : function(e){
                ...
                /*hwi*/
                var check_length = $('tags span').length; /*hwi*/
                /*hwi max 채웠을때 backspace는 먹히도록 함*/
                if ( check_length > 10 && e.key != "Backspace"){
                    e.target.textContent = '';
                    /*hwi 안내문구가 없을때만 안내문구 생성*/
                    if(!($('#upload_tag i').length > 0)) {
                        $("#upload_tag").append('<i style="color:red">&nbspUp to 10 characters & Tags can be entered</i>')
                        return;
                    } 
                    /*hwi max 채운 후 enter 키 입력 막기*/
                    if( e.key == "Enter" ){
            ...
            onInput : function(e){
                ...
                /*console.log("text size : "+ input_length);*/
                if(input_length > 15)    {                    
                    var test = value.substring(0, 15);
                    this.addTag(test);
                    e.target.textContent = '';
                    if(!($('#upload_tag i').length > 0)) {
                        $("#upload_tag").append('<i style="color:red">&nbspUp to 10 characters & Tags can be entered</i>')
        ...
        addTag : function( value ){
            ...
            return value.split(',').filter(function(v){ return !!v }).map(function(v){
                ...
                tagElm.innerHTML = "<x class='tag_x'></x><span class='tag_span'>"+ v +" </span>";
